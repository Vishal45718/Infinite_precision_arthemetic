...
namespace InfiniteArithmetic
{
	...
	{ 
		...
		if(new_a.isnegative == 1 && new_a.l == 1 && new_a.gif[0] == 0 && new_a.p == 0)
		{
			Float zero;
			return zero;
		}
...
/*			for(int i = x.p-1; i >= y.p; i--)
			...
			for(int i = 0; i < y.p; i++)
			{
				c_dec[i+1] = (x.dec[y.p-1-i] + y.dec[y.p-1-i] + c_dec[i]) / 10;
				sum.dec[i] = (x.dec[y.p-1-i] + y.dec[y.p-1-i] + c_dec[i]) % 10;
			}
			for(int i = 0; i < y.p/2; i++)
			{
				int temp = sum.dec[i];
                                sum.dec[i] = sum.dec[y.p-1-i];
                                sum.dec[y.p-1-i] = temp;
                        }
...
/*			if(x.p != y.p)
			...
			for(int i = 0; i < x.p; i++)
                        {
                                c_dec[i+1] = (x.dec[x.p-1-i] + y.dec[x.p-1-i] + c_dec[i]) / 10;
                                sum.dec[i] = (x.dec[x.p-1-i] + y.dec[x.p-1-i] + c_dec[i]) % 10;
                        }
                        for(int i = 0; i < x.p/2; i++)
                        {
                                int temp = sum.dec[i];
                                sum.dec[i] = sum.dec[x.p-1-i];
                                sum.dec[x.p-1-i] = temp;
                        }
	...
	Float operator/(Float a, Float b)
		...
		if(x.l == 1 && x.gif[0] == 0)
		{
			...
			if(x.l == 1 && x.gif[0] == 0)
				tlx = tlx - 1 - h;
                	intx = new int[tlx];
			intx_rep.l = tlx;
			intx_rep.gif = new int[tlx];
			for(int i = h; i < x.p; i++)
			{
				intx[i-h] = x.dec[i];
				intx_rep.gif[i-h] = x.dec[i];
			}
		}
		else
		{
			...
			for(int i = 0; i < tlx; i++)
				intx_rep.gif[i] = intx[i];
		}
                int* inty;
                inty = new int[y.l+y.p+n];
                int tly = y.l + y.p + n;
                for(int i = 0; i < y.l; i++)
                        inty[i] = y.gif[i];
                for(int i = y.l; i < y.l+y.p; i++)
                        inty[i] = y.dec[i-y.l];
		for(int i = y.l+y.p; i < tly; i++)
			inty[i] = 0;
		if(tly-tlx < 0)
			return zero;
                int* intdiv;
		int tldiv = tly - tlx + 1;
                if(tldiv != 0)
                        intdiv = new int[tldiv];
                for(int i = 0; i < tldiv; i++)
                        intdiv[i] = 0;
                Float fstep;
		if(tlx != 0)
                        fstep.gif = new int[tlx];
                fstep.l = tlx;
                fstep.isnegative = 0;
                for(int i = 0; i < tlx; i++)
                	fstep.gif[i] = inty[i];
			...
			if(tldiv <= PREC)
                                {
                                        ...
                                        for(int i = 0; i < div.p; i++)
                                                div.dec[i] = 0;
                                        for(int i = 0; i < tldiv; i++)
                                                div.dec[div.p-1-i] = intdiv[tldiv-1-i];
                                        new_div = zstrip(div);
                                        return new_div;
                                }
			div.p = PREC;
			div.l = tldiv - div.p;
			if(div.p != 0)
				div.dec = new int[div.p];
			if(div.l != 0)
				div.gif = new int[div.l];
			for(int i = 0; i < div.l; i++)
				div.gif[i] = intdiv[i];
			for(int i = div.l; i < tldiv; i++)
				div.dec[i-div.l] = intdiv[i];
			new_div = zstrip(div);
                        return new_div;
                }
                Float step;
                if(tlx != 0)
                        step.gif = new int[tlx+1];
                step.l = tlx + 1;
                step.isnegative = 0;
                for(int i = 0; i < tlx+1; i++)
                        step.gif[i] = 0;
                step.gif[tlx] = inty[tlx];
                for(int i = 0; i < fstep.l; i++)
                        step.gif[tlx-1-i] = fstep.gif[fstep.l-1-i];
                m = zero;
		for(int i = 0; i < 10; i++)
                {
                        m = m + one;
                ...
                m = m - one;
                intdiv[tldiv-2] = j;
                step = step - intx_rep * m;
                if(tlx+1 == tly)
                {
                        for(int i = 0; i < tldiv/2; i++)
                        {
                                int temp = intdiv[i];
                                intdiv[i] = intdiv[tldiv-1-i];
                                intdiv[tldiv-1-i] = temp;
                        }
			if(tldiv <= PREC)
                        {
                                div.l = 1;
			...
			div.p = PREC;
                ...
                Float step1;
                for(int j = 2; j < tldiv; j++)
                {
			step1.gif = new int[tlx];
			step1.l = tlx;
                        step1 = step;
			if(tlx != 0)
                                step.gif = new int[tlx+1];
                        ...
                        for(int i = 0; i < tlx+1; i++)
                                step.gif[i] = 0;
                        step.gif[tlx] = inty[tlx+j-1];
                        for(int i = 0; i < step1.l; i++)
                                step.gif[tlx-1-i] = step1.gif[step1.l-1-i];
                        m = zero;
			int k = 0;
                        for(int i = 0; i < 10; i++)
                        {
